...
const getMonthlySummary = async (req, res) => {
    try {
        ...
        const [[prevGross]] = await db.query(`
            SELECT COALESCE(SUM(s.quantity * p.sale_price), 0) AS total
            FROM sales s JOIN products p ON s.product_id = p.id
            WHERE DATE_FORMAT(s.sale_date, '%Y-%m') = DATE_FORMAT(DATE_SUB(STR_TO_DATE(CONCAT(?, '-01'), '%Y-%m-%d'), INTERVAL 1 MONTH), '%Y-%m')
        `, [month_year]);
...
const getPreviousMonthSummary = async (req, res) => {
    try {
        ...
        const [[prevMonth]] = await db.query(`
            SELECT DATE_FORMAT(DATE_SUB(STR_TO_DATE(CONCAT(?, '-01'), '%Y-%m-%d'), INTERVAL 1 MONTH), '%Y-%m') AS prev
        `, [month_year]);
