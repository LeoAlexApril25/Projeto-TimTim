  ...
  const connection = await db.getConnection(); //Usamos transação para segurança
    try{
        ...
        const [result] = await connection.query(
            ...
            [product_id, quantity, production_date, priority || 'normal', started_at || new Date(), 'em andamento']
        );
...
const getSummary = async (req, res) => {
    try{
        ...
        const [[{ total, sem_problema }]] = await db.query(`SELECT COUNT(*) as total, SUM(CASE WHEN had_issue = 0 THEN 1 ELSE 0 END) as sem_problema FROM productions WHERE status = 'finalizado'`);
...
const getStats = async (req, res) => {
    try {
        ...
        const [[{ avg_minutes }]] = await db.query(`SELECT AVG(TIMESTAMPDIFF(MINUTE, started_at, production_date)) AS avg_minutes FROM productions WHERE status = 'finalizado' AND started_at IS NOT NULL`);
        const [[{ this_week }]] = await db.query(`SELECT SUM(quantity) AS this_week FROM productions WHERE YEARWEEK(production_date, 1) = YEARWEEK(NOW(), 1)`);
        const [[{ last_week }]] = await db.query(`SELECT SUM(quantity) AS last_week FROM productions WHERE YEARWEEK(production_date, 1) = YEARWEEK(NOW(), 1) - 1`);
